...
- `review_state/<account>.json` — **the reviewer's in-progress review**, one file per LabLink account: every record's identity (tab, lab_id, sample_id, test_ids, order_id, cc_task) plus its verdict (good/bad, reason, cc_task_id) and `session_results`. Written by `UserState.persist()` on every mark, after every tab load, and by the idle reaper before it drops a session; read back by `restore_review()` whenever a `UserState` is created for that account (portal login, card login, and the timeout overlay's re-auth when the live session is gone). Snapshots older than `REVIEW_STATE_MAX_AGE_SECONDS` (12 h) are ignored. The file also carries the account's **verdict ledger** (`UserState.verdicts`, keyed `(tab, lab_id)`, each mark fresh for 12 h from when it was made): Start Pulling, Custom Day and Search rebuild their records, and `add_record()` gives a returning sample its mark, reason, listing and export row back from the ledger. So a re-pull within 12 h does not cost a mark; a next-day pull sees none. A judged sample that comes back without a COA is rendered by the window like a pending one and keeps its verdict through the render (`_render_preview`); Regenerate is the opposite, it un-judges the sample everywhere (`_reset_for_regenerate`). `tests/test_marks_follow_a_re_pull.py`. Nothing a re-render produces is stored: restored samples come back with no preview URL, unjudged ones as `pending`. Excluded from the release archive by name like the other state.
